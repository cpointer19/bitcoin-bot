...
function buildSummarySheet(trades: TradeRecord[]) {
  ...
  for (const [platform, data] of Object.entries(summary.byPlatform)) {
    rows.push({ Metric: `  ${platform} — Trades`, Value: data.count });
    rows.push({ Metric: `  ${platform} — Volume`, Value: data.volume.toFixed(2) });
    rows.push({ Metric: `  ${platform} — Net Gain/Loss`, Value: data.netGainLoss.toFixed(2) });
  }
  ...
  for (const [asset, data] of sortedAssets) {
    rows.push({ Metric: `  ${asset} — Trades`, Value: data.count });
    rows.push({ Metric: `  ${asset} — Volume`, Value: data.volume.toFixed(2) });
    rows.push({ Metric: `  ${asset} — Net Gain/Loss`, Value: data.netGainLoss.toFixed(2) });
  }
...
export async function exportTrades2025(allTrades: TradeRecord[]) {
  const trades2025 = allTrades.filter((t) => new Date(t.date).getFullYear() === 2025);
  const manualOnly = trades2025.filter((t) => t.source === "manual");

  const wb = XLSX.utils.book_new();

  const ws1 = XLSX.utils.json_to_sheet(trades2025.map(tradeToRow));
  XLSX.utils.book_append_sheet(wb, ws1, "All Trades");

  const ws2 = XLSX.utils.json_to_sheet(buildSummarySheet(trades2025));
  XLSX.utils.book_append_sheet(wb, ws2, "Summary");

  const ws3 = XLSX.utils.json_to_sheet(
    manualOnly.length > 0
      ? manualOnly.map(tradeToRow)
  ...
  a.href = url;
  a.download = "crypto-trades-2025.xlsx";
  a.click();
